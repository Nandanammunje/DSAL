...
public class BinaryHeapStarter {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < size; i++)
			a[i] = sc.nextInt();
		/*
		 * BinaryHeapEntity biHeap = new BinaryHeapEntity(size); biHeap.heapify(a);
		 * System.out.println("Enter the data to be inserted into the Heap"); data =
		 * sc.nextInt(); biHeap.insertIntoHeap(data);
		 */
		System.out.print("Enter the size of the sliding window");
		window = sc.nextInt();
		BinaryHeapOps ops = new BinaryHeapOpsImpl();
		ops.findSlidingWindow(a, window);

		/*
		 * System.out.println("Enter the rank of element"); rank=sc.nextInt();
